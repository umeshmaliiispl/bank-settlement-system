...
/**
 * PayloadValidator — Stateless, three-tier payload validator for incoming bank
 * settlement transactions in the IISPL pipeline.
 *
 * <p>
 * Validation is executed in three sequential levels. Each level runs only if
 * the previous level produced zero errors, preventing misleading cascading
 * failures:
 * <ol>
 * <li><b>Structural</b> — mandatory field presence and format checks</li>
 * <li><b>Business Rules</b> — source-type-specific amount limits and routing
 * code checks</li>
 * <li><b>Cross-Field</b> — inter-field consistency checks (e.g., same IFSC,
 * stale timestamp)</li>
 * </ol>
 *
 * <p>
 ...
 * <ul>
 * <li>Declared {@code final} and {@code private} constructor — utility class,
 * not instantiable</li>
 * <li>Reads from {@link IncomingTransaction} via getters only — input is never
 * mutated</li>
 * <li>Returns a {@link ValidationResult} value object — all errors are
 * collected before returning</li>
 * <li>Fully stateless — safe for concurrent use without synchronisation</li>
 * </ul>
 *
 * <p>
 * <b>Error Code Reference:</b>
 * 
 * <pre>
 * ┌──────────┬──────────────────────────────────────────────────────────────┐
 * │ Code     │ Meaning                                                      │
...
public final class PayloadValidator {
	...
	/**
	 * Constant zero used for amount positivity checks, avoiding repeated object
	 * creation.
	 */
	...
	/**
	 * Set of ISO 4217 currency codes accepted by the settlement pipeline. Lookup is
	 * O(1); comparison is done after upper-casing the input.
	 */
	private static final Set<String> ACCEPTED_CURRENCY_CODES = new HashSet<>(Arrays.asList("INR", "USD", "GBP", "EUR",
			"JPY", "AUD", "CAD", "SGD", "AED", "CHF", "HKD", "SEK", "NOK", "DKK"));
	...
	/**
	 * Compiled regex for Indian Financial System Code (IFSC) validation. Format: 4
	 * uppercase letters + literal '0' + 6 alphanumeric characters. Example:
	 * {@code HDFC0001234}
	 */
	private static final Pattern IFSC_CODE_PATTERN = Pattern.compile("^[A-Z]{4}0[A-Z0-9]{6}$");

	/**
	 * Compiled regex for SWIFT Bank Identifier Code (BIC) validation. Format:
	 * 4-letter bank code + 2-letter country + 2-char location + optional 3-char
	 * branch. Example: {@code HDFCINBB} or {@code HDFCINBBXXX}
	 */
	private static final Pattern SWIFT_BIC_PATTERN = Pattern.compile("^[A-Z]{4}[A-Z]{2}[A-Z0-9]{2}([A-Z0-9]{3})?$");

	/**
	 * Maximum age of a transaction timestamp before it is considered stale (30
	 * days).
	 */
	 ...
	 *
	 * <p>
	 * Each level executes only if the previous level collected zero errors, keeping
	 * error output focused on the root cause rather than cascading symptoms.
	 *
	 * @param txn        the incoming transaction payload to validate; must not be
	 *                   {@code null}
	 * @param sourceType the originating source channel (CBS, RTGS, SWIFT, NEFT,
	 *                   UPI, FINTECH)
	 * @return a {@link ValidationResult} containing the pass/fail outcome and all
	 *         collected errors
	 */
	public static ValidationResult validate(IncomingTransaction txn, SourceType sourceType) {
		...
		// Level 2 — Business Rules: source-type-specific amount limits and routing
		// codes
	...
	/**
	 * Checks that all mandatory fields are present and satisfy basic format
	 * constraints. Errors are appended to {@code validationErrors}; this method
	 * does not short-circuit.
	 *
	 * <p>
	 * Fields validated: {@code sourceRef}, {@code txnType}, {@code amount},
	 * {@code currency}, {@code valueDate}, {@code sourceSystem},
	 * {@code channelCode}, {@code txnStatus}.
	 *
	 ...
	 * <ul>
	 * <li><b>RTGS</b> — amount ≥ ₹2,00,000; valid sender/receiver IFSC; UTR must be
	 * 16 chars</li>
	 * <li><b>UPI</b> — amount ≤ ₹1,00,000; valid sender/receiver IFSC</li>
	 * <li><b>NEFT</b> — amount ≤ ₹10,00,000; valid sender/receiver IFSC</li>
	 * <li><b>SWIFT</b> — valid sender/receiver BIC (IFSC not applicable for
	 * cross-border)</li>
	 * <li><b>FINTECH</b> — net amount must equal gross minus fee; partnerName is
	 * mandatory</li>
	 ...
	 * @param validationErrors mutable list to which error codes are appended
	 */
	private static void validateBusinessRules(IncomingTransaction txn, SourceType sourceType,
	 ...
	 * <li>Transaction timestamp must not be older than
	 * {@value #STALE_TRANSACTION_THRESHOLD_DAYS} days</li>
	 * </ul>
	 *
	 * @param txn              the transaction to inspect
	 * @param validationErrors mutable list to which error codes are appended
	...
	private static void validateCrossField(IncomingTransaction txn, List<String> validationErrors) {
		...
		// Sender and receiver must not share the same IFSC (prevents same-branch
		// self-routing)
		...
		// Reject transactions with a value date older than the stale-transaction
		// threshold
		if (txn.getValueDate() != null) {
	...
	/**
	 * Validates an IFSC code against the RBI-specified format if the value is
	 * present. Blank values are silently skipped — presence is enforced separately
	 * at the structural level.
	 *
	 * @param ifscCode         the IFSC string to validate
	 * @param fieldName        the field name used in the error message (e.g.,
	 *                         "senderIfsc")
	 * @param validationErrors mutable list to which an error is appended on
	 *                         mismatch
	 */
	...
	/**
	 * Validates a SWIFT BIC code against the ISO 9362 format if the value is
	 * present. Blank values are silently skipped — presence is enforced separately
	 * at the structural level.
	 *
	...
	/**
	 * Immutable value object that carries the outcome of a
	 * {@link PayloadValidator#validate} call.
	 *
	 * <p>
	 * Contains:
	 * <ul>
	 * <li>{@code passed} — {@code true} if validation produced zero errors</li>
	 * <li>{@code errors} — unmodifiable list of structured error codes and
	 * messages</li>
	 * <li>{@code sourceType} — the source channel that was validated (used in
	 * {@code toString})</li>
	 * </ul>
	 */
	public static final class ValidationResult {
		...
		/**
		 * Ordered list of error codes and messages collected across all validation
		 * levels.
		 */
		...
		/**
		 * Returns the list of structured error codes collected during validation. The
		 * list is empty when {@link #isPassed()} is {@code true}.
		 *
		 ...
		 * <p>
		 * Example:
		 * {@code "[ING-002] sourceRef is mandatory | [ING-003] amount must be > 0"}
		 *
		 * @return pipe-delimited error summary, or an empty string if there are no
		 *         errors
		 */
		...
		public String toString() {
			return passed ? sourceType + " - VALID" : sourceType + " - INVALID → " + getErrorSummary();
		}
	}
}