 ...
 *
 * FIELDS: baseCurrency — the currency being priced e.g. USD quoteCurrency — the
 * pricing currency e.g. INR rate — mid-market rate e.g. 83.45 bidRate — bank
 * buys base at this rate e.g. 83.40 askRate — bank sells base at this rate e.g.
 * 83.50
 *
 * EXAMPLE: base=USD, quote=INR, rate=83.45 → 1 USD = 83.45 INR → convert(1000
 * USD) = 83,450 INR
 *
 * OWNED BY: Umesh (Team Lead) USED BY: Nayeem (NettingEngine calls convert()
 * for cross-currency netting) Varalaxmi (InterBankTransaction may carry FX
 * rate)
 *
...
public class ExchangeRate {
	...
	public ExchangeRate(String baseCurrency, String quoteCurrency, BigDecimal rate, BigDecimal bidRate,
			BigDecimal askRate, String source) {
		this();
	 ...
	 *
	 * Example: convert(1000) where base=USD, quote=INR, rate=83.45 → returns
	 * 83,450.00 INR
	 *
	...
	public BigDecimal convert(BigDecimal amountInBase) {
		if (amountInBase == null || rate == null) {
			throw new IllegalStateException("[ExchangeRate] Cannot convert — amount or rate is null.");
		}
	...
	/**
	 * Returns the spread = askRate - bidRate. Wider spread = more bank profit on
	 * the conversion.
	 */
	...
	public String toString() {
		return "ExchangeRate{" + getPair() + ", rate=" + rate + ", bid=" + bidRate + ", ask=" + askRate + ", date="
				+ rateDate + ", source=" + source + "}";
	}
}
