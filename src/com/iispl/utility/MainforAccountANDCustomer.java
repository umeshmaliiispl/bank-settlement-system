...
public class MainforAccountANDCustomer {
	...
	/*
	 * Date and Time formatter. Formats LocalDateTime to a clean readable format.
	 * Example Output : 09 Apr 2026 02:27:54 PM
	 *
	 * Pattern breakdown: dd → Day with leading zero : 09 MMM → Month short name :
	 * Apr yyyy → Full year : 2026 hh → Hour in 12-hour format : 02 mm → Minutes
	 * with leading zero : 27 ss → Seconds with leading zero : 54 a → AM / PM marker
	 * : PM
	 */
	...
	private static void getAllCustomers() {
		...
		/*
		 * service.getAllCustomers() calls customerDAO.getAllCustomers() which fetches
	...
	private static void getCustomerByCustomerId() {
		...
		/*
		 * service.getCustomerByCustomerId() validates input is not blank then calls
		 * customerDAO.getCustomerByCustomerId() which queries the customer table using
		 * customer_id column.
		 */
...
//            printCustomer(result.get());
			...
			System.out.println("2 No Customer Found for Customer ID : " + customerId);
		}
	}
	...
	private static void getCustomersByFullName() {
		...
		/*
		 * service.getCustomersByFullName() validates minimum 2 characters then calls
		 * customerDAO.getCustomersByFullName() which does a case insensitive partial
		 * search on full_name column.
		 */
		List<Customer> customerList = customerService.getCustomersByFullName(fullName);
	...
	private static void getActiveCustomers() {
		...
		/*
		 * service.getActiveCustomers() calls customerDAO.getAllActiveCustomers() which
		 * queries customer table filtering by customer_status = ACTIVE.
		 */
...
//        }
	...
	private static void getAllAccounts() {
		...
		/*
		 * service.getAllAccounts() calls accountDAO.findAll() which fetches all rows
	...
	private static void getAccountByAccountNumber() {
		...
		/*
		 * service.getAccountByAccountNumber() validates input is not blank then calls
		 * accountDAO.findByAccountNumber() which queries the account table using
		...
		try {
			...
			if (result.isPresent()) {
				...
				System.out.println("    No Account Found for Account Number : " + accountNumber);
			}
	...
	private static void getAccountsByBankName() {
		...
		System.out.print("  Enter Bank Name to Search" + "  (e.g. State Bank / HDFC / ICICI) : ");
		String bankName = scanner.nextLine().trim();

		/*
		 * service.getAccountsByBankName() validates minimum 2 characters then calls
		 * accountDAO.findByBankName() which does a case insensitive partial search on
		 * bank_name column.
		 */
		try {
			List<Account> accountList = accountService.getAccountsByBankName(bankName);
	...
	private static void getAccountsByAccountType() {
		...
		try {
			...
			System.out.println("    Valid Types : SAVINGS, CURRENT," + " FIXED_DEPOSIT, RECURRING_DEPOSIT, FOREX");
			return;
		}

		List<Account> accountList = accountService.getAccountsByAccountType(accountType);
	...
	private static void getAccountsByAccountStatus() {
		...
		System.out.println("    5. BLOCKED"); // ← Added
		System.out.println();
		...
		try {
			...
			System.out.println("    Valid Statuses : ACTIVE, INACTIVE, FROZEN, CLOSED, BLOCKED");
			return;
		}

		List<Account> accountList = accountService.getAccountsByAccountStatus(accountStatus);
	...
	private static void getAccountsByCustomerId() {
		...
		/*
		 * service.getAccountsByCustomerId() validates input is not blank then calls
		 * accountDAO.findByCustomerId() which queries the account table using
		 * customer_id column. One customer can have multiple accounts so List is
		 * returned.
		 */
		try {
			...
			if (accountList.isEmpty()) {
				...
				System.out.println("  Total Accounts for Customer [ " + customerId + " ] : " + accountList.size());
				System.out.println();
	...
	/*
	 * Formats a LocalDateTime value to readable string. Returns "N/A" if value is
	 * null. Example Output : 09 Apr 2026 02:27:54 PM
	 */
	...
	/*
	 * Prints all fields of a Customer object in a clean readable format. Used by
	 * all Customer READ operation methods.
	 */
	...
	/*
	 * Prints all fields of an Account object in a clean readable format. Used by
	 * all Account READ operation methods.
	 */
