...
public class TransactionServiceImpl implements TransactionService {
    ...
    public void save(IncomingTransaction incoming) {
        ...
        if (isSameBank(txn)) {
            txn = txn.toBuilder()
                    ...
                    .errorMessage("Same bank transaction not allowed")
                    .build();
        ...
        if (txn.getTxnStatus() == TransactionStatus.FAILED) {
            txn = txn.toBuilder()
                    .processingStatus(ProcessingStatus.FAILED)
                    .errorMessage("Source transaction failed at origin")
                    .build();
    ...
    public void printAllTransactions() {
        ...
        for (IncomingTransaction txn : list) {
            ...
            System.out.printf("%-4d %-20s %-8s %-8s %-20s %-20s %-12s %-6s %-12s %-14s %-40s%n",
                    ...
                    safe(txn.getTxnType()),
                    trim(txn.getSenderBankName()),
                    trim(txn.getReceiverBankName()),
    ...
    private void insert(IncomingTransaction txn, String stage) {
        ...
        if (!inserted) {
            System.out.printf("[DUPLICATE][%-18s][%-7s] REF=%-22s | SKIPPED%n",
    ...
    private String buildRemark(String stage, String error) {
        if (error == null || error.isEmpty()) return stage;
        return stage + " | " + error;
    }
